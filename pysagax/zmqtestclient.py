...
parser = argparse.ArgumentParser(description="ZMQ Test client parameters")
parser.add_argument(
    "--bin",
    metavar="N",
    type=int,
...
class ZMQConnectionThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            raw_response = self.send(command.SerializeToString())
            ...
            if raw_response is not None:
                ...
                self.console_textarea_ref.insert(tkinter.END, response)
...
class TestStreamDisplayThread(threading.Thread):
    ...
    def handle_roi_lack_of_signal_packet(
        self, packet: CoreServiceROILackOfSignalPacket
    ) -> None:
    ...
    def handle_debug_phases_packet(self, packet: CoreServiceDebugPacket) -> None:
...
class PropSetter:
    ...
    def __call__(self):
        ...
        try:
            if label == 3:  # repeated
                if type in [3, 4, 5, 6, 7, 13, 15, 16, 17, 18]:
...
class ClientWindow(tkinter.Frame):
    def __init__(self) -> None:
        ...
        self.command_builder_frame = VerticalScrolledFrame(bottom_frame)
    ...
    def send_commands(self, *args: Any) -> None:
        for setter in self.prop_setters:
            setter()
        print(self.sample_command)
        if self.zmq_thread is None or self.zmq_thread.disconnect:
            print("Not connected")
            return
    ...
    def connect_commands(self) -> None:
        ...
        self.zmq_thread.host_port = self.host_command.get().split(":")[0]
