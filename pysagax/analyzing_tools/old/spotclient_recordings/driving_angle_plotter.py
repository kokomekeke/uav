...
def plot_angles(x, column: pd.Series, window_size, aggregating_fn, name, xlabel="Samples", variance_over_line=False, plot_peaks=True, plot_dist=True, plot_quality=False, plot_df_angle=True, angle_vars=None):
...
if plot_type == 0:
    ...
    plot_angles(x_values, dataframe["df_corrected"], 1000, mean_fn, "mean azim error 1000", "Time (s)")
    plot_angles(x_values, dataframe["df_corrected"], 500, mean_fn, "mean azim error 500", "Time (s)")
    plot_angles(x_values, dataframe["df_corrected"], 100, mean_fn, "mean azim error 100", "Time (s)")
    # plot_angles(x_values, dataframe["df_corrected"], 1000, med_fn, "mean azim error 1000", "Time (s)")
    plot_angles(x_values, dataframe["df_corrected"], 500, med_fn, "med azim error 500", "Time (s)")
    plot_angles(x_values, dataframe["df_corrected"], 100, med_fn, "med azim error 100", "Time (s)")

    dataframe["keep_rows"] = dataframe["df_angle_mean"].diff()
    dataframe["keep_rows"][0] = 1
    dataframe = dataframe[dataframe["keep_rows"] != 0]
    x_values = dataframe["seconds"]
    dataframe["df_angle"] = dataframe["df_corrected_mean"]
...
if plot_type == 2:
    ...
    df_ax.plot(dataframe["seconds"], dataframe["df_corrected_mean"]*180/np.pi, "r", label="mean azimuth")
    pass
else:
...
df_aggregated_heading_error = dataframe["df_corrected_mean"] - dataframe["azim"]
