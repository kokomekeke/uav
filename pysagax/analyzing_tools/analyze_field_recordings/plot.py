...
def plot_mean(
  ...
  correct_values_label,
    ...
    ax.plot(x_values, correct_values, color="green", label=correct_values_label, zorder=10, linewidth=4)
...
def plot_peaks(
  ...
  peaks, # pandas DataFrame containing columns ["peaks0", "peaks1", etc]
    ...
    peaks['maxpeak2'] = peaks['maxpeak2'].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000)
    peaks['minpeak2'] = peaks['minpeak2'].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000)
    peaks['minpeak2'] = peaks['minpeak2'].map(lambda x: -1 if x > -1 else x)   #make the peak band visible if all channels have full scale peak values
    ...
    # TODO: individual channel peaks with thin red blue yellow green lines


    ax.plot(x_values, peaks["peaks0"].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000), color="gold", label="ch0", zorder=1, alpha=0.5,)
    ax.plot(x_values, peaks["peaks1"].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000), color="blue", label="ch1", zorder=10,   alpha=0.5,)
    ax.plot(x_values, peaks["peaks2"].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000), color="green", label="ch2", zorder=1,  alpha=0.5,)
    ax.plot(x_values, peaks["peaks3"].map(lambda x: 20 * math.log10(x / adc_resolution) if x > 0 else -1000), color="red", label="ch3", zorder=10,   alpha=0.5,)

    ax.fill_between(x=x_values, y1=peaks["maxpeak2"], y2=peaks["minpeak2"], color="black", alpha=0.2, linestyle="None", linewidth=0, label="range of peak dBFS for ch0..ch3")
...
def main(path: str, save_png:bool = False):
    """
    This plots!
    TODO  
    """
    ...
    plot_peaks(
        ...
        x_values=x_values,
        peaks=detection_df[["peaks0", "peaks1", "peaks2", "peaks3"]], # pandas DataFrame containing columns ["peak0", "peak1", etc]
    )
